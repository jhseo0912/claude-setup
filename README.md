...
새 기기나 새 계정에서 `bash install.sh` 한 번이면 그대로 복원된다.
...
기존 `~/.claude/CLAUDE.md`는 `.bak`으로 백업되고, settings.json은 hook만 추가되며 나머지 설정은 유지된다.
...
| `/distill` | md가 비대해졌을 때 | 서사 로그를 제약·결정(ADR)·상태·docs 4갈래로 증류한다 |
...
| `hooks/md-discipline.sh` | 지식 md를 고칠 때 압축 규칙 리마인더를 넣는 PostToolUse hook |
...
`~/.claude/vendor/im-not-ai`로 클론해 그쪽 설치 스크립트에 맡긴다. 정본은 upstream이다.
...
정본은 이 리포다. `~/.claude/`를 직접 고쳤으면 여기로 복사해 커밋한다. 두 곳에 두면 갈린다.
